...
class licensePlate:
    ...
    def __init__(self, onnx_file, ort_provider=['CUDAExecutionProvider', 'CPUExecutionProvider']):
        ...
        self.count=0
        self.result = ''
        self.line = LINE("G2lF09ZOliMM3l7F7I8KIYM1JRDQDQ9NhgPx6G5GYXY")
        self.provinces = ["กรุงเทพมหานคร", "สมุทรปราการ", "นนทบุรี", "ปทุมธานี", "พระนครศรีอยุธยา",
                    ...
                    "ตาก", "สุโขทัย", "พิษณุโลก", "พิจิตร", "เพชรบูรณ์", "ราชบุรี",
                    "กาญจนบุรี", "สุพรรณบุรี", "นครปฐม", "สมุทรสาคร", "สมุทรสงคราม",
                    "เพชรบุรี", "ประจวบคีรีขันธ์", "นครศรีธรรมราช", "กระบี่", "พังงา", "ภูเก็ต",
                    "สุราษฎร์ธานี", "ระนอง", "ชุมพร", "สงขลา", "สตูล", "ตรัง", "พัทลุง", "ปัตตานี",
    ...
    def check_province(self,misspell):
    
        misspell_value = []
        for i in self.provinces:
            similarity_ratio = SequenceMatcher(None, i, misspell).ratio()
            misspell_value.append(similarity_ratio)
        most_similar = self.provinces[np.argmax(misspell_value)]
    ...
    def test(self, img):
        ...
        if len(result) > 0:
            self.province = self.check_province(province)
    
            self.show_result = result+' '+self.province
            print('true = ',self.show_result)
        self.real_result = result+' '+self.province
        self.result = result
    ...
    def pil2cv2(self,img,text,bbox):
        ...
        font_path = r"C:\Windows\Fonts\angsana.ttc"  # or any other path where the font is installed
                ...
                if self.list[0] == self.list[1]:
                    ...
                    if os.path.isfile('ocr_results.csv'):
                        ...
                        with open('ocr_results.csv', 'a', encoding='utf-8') as file:
                            ...
                            writer.writerow(data)
                    license_plate.line_noti(self.real_result)
                    self.check = 0
                    self.list = []
                else:
                    self.list = []
    ...
    def check_plate(self, input_tensor, frame):
        ...
        cv2.putText(image_preview,'Complete!',(300, 70), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), 3)
        cv2.putText(image_preview,'TAKE A LICENSE PLATE OUT  ! ',(10, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        if len(boxes) == 0:
            self.count += 1
        if len(boxes) > 0:
            self.count = 0
    ...
    def line_noti(self, string):
        return self.line.sendtext(string)
...
while True :
    ...
    if license_plate.check == 1:
        ...
        image_preview = license_plate.preview(frame, boxes, num_box)
        cv2.putText(image_preview,'Processing...',(300, 70), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 3)
        text = ''
